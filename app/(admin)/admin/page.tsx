...
import { getAdminStats, getAdminPaymentsData, getAdminTransData } from "@/app/actions/action";
...
async function AdminHome() {
    ...
    if (!session) { return }
    const res: statsType = await getAdminStats()
    const stats = res.stats;

    const transactions = await getAdminTransData()
    const payments = await getAdminPaymentsData()
    ...
    return (
        <div className="w-full h-full flex flex-col space-y-10">
            <h1 className="text-lg text-primary-500 font-bold">Dasboard</h1>
            <div className="w-full flex  justify-around">
                {statsData.map((stat, index) => (
                    <GridStats key={index} {...stat} />
                ))}
            </div>
            <h1 className="text-xl text-primary-500">Customers Transactions</h1>
            <ClientTransChart transactions={transactions.result} />

            <h1 className="text-xl text-primary-500">Customers Payments</h1>
